...
bool menuOpen = false;
// 2 pages: page 0 is STATS/POKEDEX/SETTINGS/RETIRE (unchanged), page 1 adds
...
// Same reason: the main loop's tick() resolves a finished capture ~3300 lines
// above btlFinishCapture()'s own definition, and renderBattle() shows the
...
Pet btlWildFoe;   // the scratch Pet behind btlFoe during a wild fight, kept
                  // alive for the whole battle -- a successful capture needs
                  // its real IVs/moves, which Combatant does not carry
bool btlOver = false;
bool btlWon = false;
...
// Wild capture. The outcome is rolled the instant the ball is thrown --
...
void setup() {
  ...
  bool touchOk = false;
  for (int i = 0; i < 3 && !touchOk; i++) {  // a veces falla al primer intento
    touchOk = touch.begin(Wire, 0x38, IIC_SDA, IIC_SCL);
    if (!touchOk) delay(150);
  }
  if (!touchOk) Serial.println("FT3168 no detectado");
  // begin() deja el chip en modo comando (lee la identidad y no sale);
  // hace falta un reset por hardware para que vuelva a reportar toques
  if (TP_RESET >= 0) touch.reset();
  touch.setMaxCoordinates(LCD_WIDTH, LCD_HEIGHT);
...
void loop() {
  ...
  // The pokeball's throw/shake/result animation runs on its own clock; once
  // it closes, resolve the capture BEFORE the endedKind check just below so a
  // caught creature's endedMon is visible to it on this same frame.
  if (btlBallUntil && now >= btlBallUntil) btlFinishCapture();
  ...
  if (pet.endedKind != CER_NONE && !partyPick) {
    ...
    if (party.add(pet.endedMon) || party.boxAdd(pet.endedMon)) {
      snprintf(partyBannerName, sizeof(partyBannerName), "%s",
               pet.endedMon.nick[0] ? pet.endedMon.nick : dexName(pet.endedMon.dex));
...
void handleTouch() {
  ...
  // que detectar el levantamiento). Leer el touch dormido se colgaba ~1s y
  // congelaba el loop entero; SensorLib no respeta el timeout de Wire.
  // PORTAGE 1.43: si TP_INT no esta cableado (ver pin_config.h), no hay IRQ
  // que gatee la lectura -- se pollea el bus siempre a 50Hz en su lugar.
  if (TP_INT >= 0 && !gTouchIrq && !wasPressed) return;
  gTouchIrq = false;
  int16_t x, y;
...
void renderMonSheet(const PartyMon &m, bool fromBox) {
  ...
  snprintf(head, sizeof(head), "%s%s Lv.%u", m.shiny ? "*" : "",
           m.nick[0] ? m.nick : dexName(m.dex), (unsigned)m.level);
  ...
  if (releaseConfirm) {
    char q[40];
    snprintf(q, sizeof(q), T(S_RELEASE_FMT), m.nick[0] ? m.nick : dexName(m.dex));
...
void onSwipe(int dir) {
  ...
  if (pet.awaitingStarter()) return;  // bloqueado durante la eleccion de inicial
  if (menuOpen) {   // horizontal pages between the 2 menu pages; vertical still closes
    int p = (int)menuPage + (dir > 0 ? -1 : 1);
    if (p >= 0 && p < MENU_PAGES) menuPage = (uint8_t)p;
    return;
  }
  if (battleOpen) return;   // no swiping out of a fight
...
void onTap(int16_t x, int16_t y) {
  ...
  if (menuOpen) {
    ...
    for (int i = 0; i < MENU_ROWS; i++) {
      ...
      if (i == MENU_ROWS - 1) {   // CLOSE, same slot on every page
        sfxPlay(SFX_TAP);
        menuOpen = false;
        return;
      }
...
// Same shape as foeFromSpecies, except the IVs are left exactly as
// dbgHatchAs() -> hatch() -> rollIVs() naturally rolls them (8-31, no
// careBonus push, since the scratch Pet starts at zero care state) rather
// than being overwritten with a flat EASY_IV/HARD_IV -- a wild creature has
// no care history to push them and should have the same natural spread a
// real egg would. `outFoe` is kept alive by the caller for the whole fight:
// a successful capture needs its real IVs and moves, which Combatant does
// not carry. Not reset via assignment (`outFoe = Pet()`) -- Pet embeds a
// Preferences, and the emulator's host stub gives it a reference member that
// deletes Pet's assignment operator, so that compiles on real ESP32 but not
// here. dbgHatchAs() -> hatch() already overwrites every field that matters
// (species, IVs, moves), same as foeFromSpecies's own freshly-constructed
// scratch Pet.
static void wildFoeFromSpecies(Combatant &c, Pet &outFoe, int16_t dex, uint8_t lvl) {
  outFoe.dbgHatchAs(dex, false);
  outFoe.ageMinutes = (uint32_t)(lvl ? lvl - 1 : 0) * MINUTES_PER_LEVEL;
  outFoe.relearnFromLevel();
  combatantFromPet(c, outFoe);
}
...
// Shared setup for every way a battle can start. Each start*Battle() builds
// btlYou/btlFoe (and whatever squad state it needs) FIRST, then calls this,
// then sets its own distinguishing field (btlTrainer/btlLink/btlWild)
// explicitly -- so none of the four can ever inherit a stale flag left over
// from whichever battle ran before it. This is what used to be ~15 lines
// copy-pasted into three separate functions; the copy was how startBattle()
// ended up never resetting btlTrainer at all (fixed here, by construction).
static void btlResetCommon() {
  btlTrainer = -1;
  btlLink = false;
  btlWild = false;
  btlMsgCount = 0;
...
// How far a wild encounter's level can stray from the player's own live
// pet, in either direction. A balance constant, like the ladder's IVs.
#define WILD_LEVEL_SPREAD 3

// A wild fight: not part of the gated ladder (no level/size cap, same as the
// debug BATTLE command above), against a single R_COMUN creature of the
// player's current region, close to their own level. Offers CAPTURE.
void startWildBattle() {
  if (pet.isEgg() || pet.ceremony != CER_NONE) return;
  int16_t dex = pet.rollInRegion(pet.region % REGION_COUNT, R_COMUN);
  int lvl = (int)pet.level() + (int)random(2 * WILD_LEVEL_SPREAD + 1) - WILD_LEVEL_SPREAD;
  if (lvl < 1) lvl = 1;
  if (lvl > MAX_LEVEL) lvl = MAX_LEVEL;
  buildSquad(0, TRAINER_TEAM_MAX, 0xFFFF);
  if (!btlSquadN) return;
  wildFoeFromSpecies(btlFoe, btlWildFoe, dex, (uint8_t)lvl);
  btlResetCommon();
  btlWild = true;
  battleOpen = true;
}
...
void renderBattle() {
  if (btlBallUntil) { renderCapture(); return; }
  ...
  if (lanWait && !btlMsgCount) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      gfx->setTextColor(UI_INK);
      gfx->setTextSize(2);
      gfx->setCursor(x + (BTL_CELL_W - (int)strlen(lbl[i]) * 12) / 2, y + 14);
      gfx->print(lbl[i]);
    }
  } else if (btlMenu == 0) {
...
// closes (see the tick() call site). A miss costs the turn -- btlResolve(0)
// is the same "move 0 = no attack" idiom btlSwitchTo() already uses, so the
// wild foe simply gets its turn. A catch closes the battle immediately and
// hands off to the SAME generic "a creature needs a slot" flow a farewell
// already uses (TamaPoke.ino, the pet.endedKind check near the top of the
// main loop): party first, then the box, then a manual choice if both are
...
static void btlFinishCapture() {
  ...
  btlFreeSprites();
  audioMusic(MUS_NONE);
  PartyMon m;   // built by hand, NOT via Pet::snapshotForParty() -- that is
                // gated on the LIVE pet's own ceremony state and has nothing
                // to do with a wild capture
  m.dex = btlWildFoe.speciesId;
...
// Plain Arduino_GFX primitives, not a sprite -- a capture has no species of
// its own and no licensing question. Arcs from the player's corner to the
// foe's over the first third of BALL_ANIM_MS, shakes over the middle third,
// and flashes the result (the same expanding-ring halo renderWin() uses for
// a hard-mode badge) over the last third.
void renderCapture() {
  gfx->fillScreen(RGB565_BLACK);
  drawBattleBack();
  ...
  uint32_t now = millis();
  uint32_t elapsed = now >= btlBallUntil ? BALL_ANIM_MS : BALL_ANIM_MS - (btlBallUntil - now);
  int third = BALL_ANIM_MS / 3;
  int bx, by;
  if ((int)elapsed < third) {
    // integer parabolic arc from your corner to the foe's
    int t1000 = (int)elapsed * 1000 / third;
    int x0 = 96, y0 = 320, x1 = 300, y1 = 90;
    bx = x0 + (x1 - x0) * t1000 / 1000;
    by = y0 + (y1 - y0) * t1000 / 1000 - (80 * t1000 * (1000 - t1000)) / 1000000;
  } else if ((int)elapsed < 2 * third) {
    int shakeT = (int)elapsed - third;
    bx = 300 + ((shakeT / 90) % 2 ? 5 : -5);
    by = 90;
  } else {
    bx = 300; by = 90;
    int flashT = (int)elapsed - 2 * third;
    if (btlCaptureWon) {
      for (int r = 6 + flashT / 30; r >= 0; r -= 6)
  ...
  if ((int)elapsed >= 2 * third) {
    ...
    gfx->setTextColor(UI_INK);
    gfx->setTextSize(1);
    gfx->setCursor(CX - (int)strlen(msg) * 3, BTL_GRID_Y + 18);
    gfx->print(msg);
  }
  gfx->flush();
}
...
void battleTap(int16_t x, int16_t y) {
  if (btlBallUntil) return;   // the throw/shake/result animation ignores taps
...
void drawMenu() {
  ...
  // page dots, in the panel's top margin -- there is no room left for them
  // below the rows (CLOSE's own bottom margin is already tuned to the bezel)
  for (int i = 0; i < MENU_PAGES; i++) {
    int dx = CX - (MENU_PAGES - 1) * 7 + i * 14;
    if (i == menuPage) gfx->fillCircle(dx, MENU_Y + 8, 3, UI_INK);
    else gfx->drawCircle(dx, MENU_Y + 8, 2, UI_INK);
  }
...
void renderBox() {
  ...
  if (boxSwapFrom) {
    ...
    snprintf(sub, sizeof(sub), T(S_BOX_SWAP),
             p.empty() ? "-" : (p.nick[0] ? p.nick : dexName(p.dex)));
    gfx->setTextColor(UI_BAR_WARN);
    gfx->setTextSize(1);
  ...
  for (uint8_t i = 0; i < BOX_PER_PAGE; i++) {
    ...
    gfx->print(m.nick[0] ? m.nick : dexName(m.dex));
    char l[16];
