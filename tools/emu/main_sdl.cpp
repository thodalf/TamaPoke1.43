...
#ifdef _WIN32
  // No <sys/select.h>/fd_set on native Windows, and console stdin isn't a
  // select()-able descriptor at all -- _kbhit()/_getch() is the MinGW/MSVC
  // stand-in, at the cost of doing our own line editing (echo, backspace)
  // that a POSIX terminal's canonical mode would otherwise give for free.
  while (_kbhit()) {
    int c = _getch();
    if (c == '\r' || c == '\n') {
      putchar('\n');
      g_lines.push_back(g_partial);
      g_partial.clear();
    } else if (c == 8 || c == 127) {
      if (!g_partial.empty()) { g_partial.pop_back(); fputs("\b \b", stdout); }
    } else {
      putchar(c);
      g_partial += (char)c;
    }
